...
static inline void get_literal_code(const isal_hufftables* const huffman_table_ptr, const uint32_t literal,
                                    uint32_t& code, uint32_t& code_length) {
    code        = huffman_table_ptr->lit_table[literal];
    code_length = huffman_table_ptr->lit_table_sizes[literal];
}

void update_state(huffman_only_state<execution_path_t::software>& stream, uint8_t* start_in_ptr, uint8_t* next_in_ptr,
                  uint8_t* end_in_ptr) noexcept {
    auto isal_state = &stream.isal_stream_ptr_->internal_state;

    if (next_in_ptr - start_in_ptr > 0) { isal_state->has_hist = IGZIP_HIST; }
...
auto huffman_only_compress_block(huffman_only_state<execution_path_t::software>& stream,
    ...
    while (next_in_ptr + ISAL_LOOK_AHEAD < end_in_ptr) {
        ...
        get_literal_code(stream.isal_stream_ptr_->hufftables, literal & max_uint8, literal_code, code_length);
