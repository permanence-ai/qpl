...
static void fill_src_buffer_32u(uint8_t* src, uint8_t* dst, uint32_t length, uint32_t nbits) {
    ...
    std::transform(p_src_32u, p_src_32u + length, p_dst_32u, [&mask](auto x) { return x & mask; });
}
...
QPL_UNIT_API_ALGORITHMIC_TEST(qplc_unpack_be_32u, base) {
        ...
        for (uint32_t length = 1; length <= TEST_BUFFER_SIZE; length++) {
            for (uint32_t start_bit = 0; start_bit < 32; start_bit++) {
                ...
                ASSERT_TRUE(CompareSegments(reference.begin(), reference.end(),
                    destination.begin(), destination.end(), "FAIL qplc_unpack_be_32u!!! "));
            }
        }
    }
}
