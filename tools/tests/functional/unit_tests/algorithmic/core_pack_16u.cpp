...
void static ref_qplc_pack_16unu(const uint8_t* src_ptr, uint32_t num_elements, uint8_t* dst_ptr, uint32_t start_bit, uint32_t nbits)
{
...
{
    ref_qplc_pack_16u9u,
    ref_qplc_pack_16u10u,
    ref_qplc_pack_16u11u,
    ref_qplc_pack_16u12u,
    ref_qplc_pack_16u13u,
    ref_qplc_pack_16u14u,
        ...
        for (uint32_t length = 1; length <= TEST_BUFFER_SIZE; length++) {
            for (uint32_t start_bit = 0; start_bit < 16; start_bit++) {
                ...
                ASSERT_TRUE(CompareSegments(reference.begin(), reference.end(),
                    destination.begin(), destination.end(), "FAIL qplc_pack_16u!!! "));
            }
        }
    }
}
