...
static uint32_t ownc_decode_prle_header(uint8_t** pp_src, const uint8_t* src_stop_ptr, uint32_t* format_ptr, uint32_t* count_ptr) {
...
static uint32_t ref_qplc_unpack_prle_8u(uint8_t** pp_src, uint32_t src_length, uint32_t bit_width,
    uint8_t** pp_dst, uint32_t dst_length, int32_t* count_ptr,  uint32_t* value_ptr) {
    uint32_t count = 0U;
    uint32_t format = 0U;
    uint8_t  value = 0U;
...
static uint32_t ref_qplc_unpack_prle_16u(uint8_t** pp_src, uint32_t src_length, uint32_t bit_width,
    uint8_t** pp_dst, uint32_t dst_length, int32_t* count_ptr, uint32_t* value_ptr) {
    uint32_t count = 0U;
    uint32_t format = 0U;
    uint16_t value = 0U;
    ...
    if (0 < *count_ptr) {
        count = std::min(static_cast<uint32_t>(*count_ptr),
                         static_cast<uint32_t>(dst_length / sizeof(uint16_t)));
        *count_ptr = *count_ptr - (int32_t)count;
...
static uint32_t ref_qplc_unpack_prle_32u(uint8_t** pp_src, uint32_t src_length, uint32_t bit_width,
    uint8_t** pp_dst, uint32_t dst_length, int32_t* count_ptr, uint32_t* value_ptr) {
    uint32_t count = 0U;
    uint32_t format = 0U;
    uint32_t value = 0U;
    ...
    if (0 < *count_ptr) {
        count = std::min(static_cast<uint32_t>(*count_ptr),
                         static_cast<uint32_t>(dst_length / sizeof(uint32_t)));
        *count_ptr = *count_ptr - (int32_t)count;
        ...
        for (uint32_t length = 1U; length <= TEST_BUFFER_SIZE; length++) {
            for (uint32_t length_dst = 1U; length_dst <= TEST_BUFFER_SIZE; length_dst++) {
                for (int32_t count = -1; count <= (int32_t)TEST_BUFFER_SIZE; count++) {
                    ...
                    ASSERT_EQ(((uint8_t*)p_value - (uint8_t*)value.data()), ((uint8_t*)p_ref_value - (uint8_t*)ref_value.data()));
                    ASSERT_TRUE(CompareSegments(reference.begin(), reference.end(),
                        destination.begin(), destination.end(), "FAIL qplc_unpack_prle_8u!!! "));
                }
            }
        }
    }
        ...
        for (uint32_t length = 1U; length <= TEST_BUFFER_SIZE; length++) {
            for (uint32_t length_dst = 1U; length_dst <= TEST_BUFFER_SIZE; length_dst++) {
                for (int32_t count = -1; count <= (int32_t)TEST_BUFFER_SIZE; count++) {
                    ...
                    ASSERT_EQ(((uint8_t*)p_value - (uint8_t*)value.data()), ((uint8_t*)p_ref_value - (uint8_t*)ref_value.data()));
                    ASSERT_TRUE(CompareSegments(reference.begin(), reference.end(),
                        destination.begin(), destination.end(), "FAIL qplc_unpack_prle_16u!!! "));
                }
            }
        }
    }
        ...
        for (uint32_t length = 1U; length <= TEST_BUFFER_SIZE; length++) {
            for (uint32_t length_dst = 1U; length_dst <= TEST_BUFFER_SIZE; length_dst++) {
                for (int32_t count = -1; count <= (int32_t)TEST_BUFFER_SIZE; count++) {
                    ...
                    ASSERT_EQ(((uint8_t*)p_value - (uint8_t*)value.data()), ((uint8_t*)p_ref_value - (uint8_t*)ref_value.data()));
                    ASSERT_TRUE(CompareSegments(reference.begin(), reference.end(),
                        destination.begin(), destination.end(), "FAIL qplc_unpack_prle_32u!!! "));
                }
            }
        }
    }
